...
# This is important on Render because the SQLite database
# may not exist yet when the service starts.
init_db()
...
def _load_history(db: Session, conversation_id: int) -> list:
    """
    Reconstruct prior turns as the {role, content} dictionaries
...
def chat(
    ...
    # Persist the incoming user message before calling the agent,
    # so it is never lost even if the model call fails.
    user_msg = Message(
    ...
    try:
        ...
        # Log the failure as a flagged conversation for the
        # business team rather than losing the turn silently.
        convo.flagged = True
...
def current_conversation(
    ...
    so the chat UI can restore it instead of starting a blank thread every
    time a contact is selected or the page is reloaded.
    """
